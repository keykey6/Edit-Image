  ...
  <div class="tool-page">
    <div class="tool-upload">
      <div class="upload-area" @click="triggerInput">
        ...
        <p class="upload-hint">统一调整为指定尺寸</p>
      </div>
    ...
    <div class="tool-params">
      ...
      <div class="param-row">
        ...
        <el-radio-group v-model="params.mode" size="small">
          <el-radio-button value="cover">裁切填充</el-radio-button>
          <el-radio-button value="contain">留白适应</el-radio-button>
          <el-radio-button value="stretch">拉伸变形</el-radio-button>
        </el-radio-group>
      </div>
    </div>
    ...
    <div v-if="files.length" class="tool-actions">
      <el-button type="primary" size="large" round :loading="processing" @click="handleProcess">
        {{ processing ? '处理中...' : `调整 ${files.length} 张图片` }}
      </el-button>
    </div>
    ...
    <div v-if="resultUrl" class="tool-result">
      <el-result icon="success" title="批量调整完成">
        <template #extra>
...
const params = reactive({ width: 800, height: 600, mode: 'cover' })
...
async function handleProcess() {
  ...
  try {
    ...
    const res = await api.post('/batch-resize/process', fd, { responseType: 'blob' })
...
function handleDownload() {
  ...
  const a = document.createElement('a'); a.href = url; a.download = 'resized.zip'; a.click()
